...
- Luôn lắng nghe nhu cầu khách hàng và phản ứng thích ứng: nếu khách muốn tư vấn thì tư vấn, nếu muốn mua liền thì mua liền.
- Có quyền "tám" chuyện ngoài lề về công nghệ để tăng thiện cảm và xây dựng niềm tin.

📋 HAI FLOW CHÍNH (Linh hoạt, không cứng nhắc):

## ✅ FLOW 1: TƯ VẤN CHUYÊN SÂU (Khi khách muốn tìm hiểu trước khi mua)
Nhận diện: Khách hỏi "nên mua loại nào", "có gì khác nhau", "loại nào tốt hơn", "hợp cho dùng gì"...

**Bước 1: Hỏi & Hiểu nhu cầu**
- Đặt câu hỏi thông minh: "Bạn dùng để làm gì?" "Ngân sách khoảng bao nhiêu?" "Ưu tiên tính năng nào?"
- Lắng nghe kỹ để hiểu rõ nhu cầu thực sự

**Bước 2: Gợi ý dựa trên kiến thức chuyên môn**
- Giải thích khác biệt giữa các sản phẩm (cấu hình, giá, ưu nhược điểm)
- Recommend cách tối ưu nhất cho nhu cầu của khách

**Bước 3: Tìm kiếm sản phẩm phù hợp**
- Khi khách muốn xem, gọi 'searchProducts' với keyword từ khách yêu cầu
- Frontend hiển thị cards các sản phẩm - hướng dẫn khách lựa chọn

**Bước 4 → Chuyển sang FLOW 2 (Mua hàng) khi khách chọn xong**

---

## ⚡ FLOW 2: MUA HÀNG LIỀN (Khi khách sẵn sàng order ngay)
Nhận diện: Khách nói "tôi muốn mua", "cho tôi cái này", "tìm giúp tôi loại...", "có cái giá...không"...

**Bước 1: Lắng nghe & Xác định sản phẩm**
- Khách có thể mô tả theo: tên sản phẩm, giá tiền, yêu cầu đặc biệt, màu/dung lượng...
- KHÔNG yêu cầu PHẢI có đầy đủ màu + dung lượng. Linh hoạt thích nghi!

**Bước 2: Tìm kiếm theo yêu cầu (Flexible)**

  *Trường hợp 2.1: Khách muốn theo tên sản phẩm*
  - Gọi searchProducts với tên (VD: "iPhone 15")
  - Frontend hiển thị danh sách variants (tất cả màu/dung lượng) → khách chọn
  ...
  *Trường hợp 2.3: Khách nêu yêu cầu đặc biệt (camera tốt, pin lâu, etc)*
  ...
  - Sau đó search sản phẩm theo tên/loại, không rườm rà

**Bước 3: Xác nhận sản phẩm & Lấy thông tin khách hàng**
- Khi khách đã chọn 1 variant cụ thể (hoặc xác nhận bằng miệng)
- Thu thập: Tên người nhận, SĐT, Email (nếu có), Địa chỉ chi tiết, Số lượng
- CÓ THỂ HỎI TỪ ĐỀU VÀO 1 TIN NHẮN HOẶC TỪ TỪNG CÁI MỘT - tùy theo flow tự nhiên

**Bước 3.5: Hiển thị phương thức thanh toán**
- Gọi 'getPaymentMethods'. Frontend hiển thị UI
- Hỏi: "Bạn thanh toán bằng cách nào ạ?"

**Bước 4: Xác nhận & Tạo đơn**
- Confirm lại toàn bộ: "Tên [X], SĐT [Y], Địa chỉ [Z], thanh toán [phương thức], đúng không?"
- Khi khách xác nhận (nói "đúng", "ok", "được", "xong", "tạo", etc) → GỌI 'createOrderRequest' NGAY
...
🎭 **QUY TRÌNH NHẬN DIỆN**:
- Nếu khách NÊU NGAY cách mua (tên/giá/yêu cầu) → FLOW 2 (Mua liền, không tư vấn dài)
- Nếu khách **HỎI HỎI** trước khi quyết định → FLOW 1 (Tư vấn trước)
- Luôn **LẮNG NGHE HƠN LÀM LỨC** - khách hàng giá trị nhất là thời gian
...
const tools = [
  {
    ...
    function: {
      name: "searchProducts",
      description: "Linh hoạt tìm kiếm sản phẩm theo: TÊN sản phẩm, GIÁ TIỀN, YÊU CẦU ĐẶC BIỆT. KHÔNG yêu cầu phải có màu & dung lượng - chỉ cần keyword chính. VD: 'iPhone 15' hoặc 'smartphone dưới 10 triệu' hoặc 'camera tốt'.",
      parameters: {
        type: "object",
        properties: {
          keyword: { type: "string", description: "Keyword chính: tên sản phẩm (iPhone 15, Macbook), loại (smartphone, laptop), yêu cầu đặc (camera tốt, pin lâu) hoặc giá (dưới 10 triệu, từ 15-20 triệu). KHÔNG chứa màu/dung lượng ở đây." },
          color: { type: "string", description: "Màu sắc (nếu khách yêu cầu). Để trống nếu không có." },
          storage: { type: "string", description: "Dung lượng (nếu khách yêu cầu). Để trống nếu không có." },
          priceRange: { type: "object", description: "Nếu khách tìm sản phẩm theo giá (VD: 'dưới 10 triệu', '15-20 triệu'). Để trống nếu không tìm theo giá.", properties: { min: { type: "number", description: "Giá tối thiểu (VND)" }, max: { type: "number", description: "Giá tối đa (VND)" } } },
        },
        required: ["keyword"],
      },
    },
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "⭐ PHẢI GỌI NGAY KHI KHÁCH XÁC NHẬN CUỐI CÙNG ⭐ Tạo yêu cầu đặt hàng từ chatbot. Gọi tool này khi user nói 'đúng', 'ok', 'được', 'xong', hoặc bất kỳ lời xác nhận nào SAU KHI ĐÃ CHỌN PAYMENT. NẾU KHÔNG GỌI TOOL NÀY, CHATBOT SẼ KHÔNG THỂ TẠO ĐƠN.",
      parameters: {
        type: "object",
        properties: {
...
// 👉 TÌM SẢN PHẨM
const executeSearchProducts = async (args: { keyword: string; color?: string; storage?: string; priceRange?: { min?: number; max?: number } }) => {
  try {
    const { keyword, color, storage, priceRange } = args;
    ...
    console.log(`🔍 Search keyword: "${keyword}" | Found ${products.length} products | priceRange: ${JSON.stringify(priceRange)}`);
...
Ví dụ: "Mình chưa bán dòng đó, nhưng nếu bạn cần một laptop hiệu năng tốt dưới 15 triệu, mình có thể gợi ý..."`,
    ...
    let exactPriceMatchFound = false;
    let priceMatchMessage = "";
    ...
    products.forEach((p: any) => {
      p.variants.forEach((v: any) => {
        ...
        v.variantAttributes?.forEach((va: any) => {
          ...
          if (["color", "màu", "màu sắc"].includes(attrCode) || ["color", "màu", "màu sắc"].includes(attrName)) {
            colorObj.label = va.attributeOption?.label;
            colorObj.value = va.attributeOption?.value;
          }

          if (["storage", "dung lượng", "rom", "rom_capacity"].includes(attrCode) || ["storage", "dung lượng", "rom", "rom_capacity"].includes(attrName)) {
        ...
        // 🔥 THÊM: Filter theo giá tiền nếu có priceRange
        const originalPrice = Number(v.price);
        const { finalPrice } = calculateDiscountPrice(originalPrice, p.id, p.categoryId, p.brandId, activePromotions);
        
        if (priceRange) {
          // Kiểm tra xem finalPrice có nằm trong priceRange không
          const inRange = (!priceRange.min || finalPrice >= priceRange.min) && (!priceRange.max || finalPrice <= priceRange.max);
          
          if (inRange) {
            exactPriceMatchFound = true;
            console.log(`✅ Tìm thấy giá trúng: ${finalPrice} (${priceRange.min || 0} - ${priceRange.max || Infinity})`);
          } else {
            // ❌ Giá không match - bỏ qua variant này nhưng lưu lại để gợi ý sau
            return;
          }
        }
        ...
        const { hasPromotion } = calculateDiscountPrice(originalPrice, p.id, p.categoryId, p.brandId, activePromotions);
    ...
    // 🔥 THÊM: Nếu tìm kiếm theo giá nhưng không tìm thấy exact match, gợi ý giá gần nhất
    if (priceRange && variants.length === 0) {
      console.log(`⚠️ Không tìm thấy giá trúng, đang tìm giá gần nhất...`);
      const priceRangeMin = priceRange.min || 0;
      const priceRangeMax = priceRange.max || Infinity;
      const tolerance = 1000000; // 1 triệu VND tolerance
      
      // Lấy tất cả variants và sort theo giá có độ lệch nhỏ nhất
      const allVariants: any[] = [];
      products.forEach((p: any) => {
        p.variants.forEach((v: any) => {
          const originalPrice = Number(v.price);
          const { finalPrice } = calculateDiscountPrice(originalPrice, p.id, p.categoryId, p.brandId, activePromotions);
          allVariants.push({ ...v, product: p, finalPrice, originalPrice });
        });
      });

      // Sort tìm giá gần nhất trong tolerance
      allVariants.sort((a, b) => {
        const midPrice = (priceRangeMin + priceRangeMax) / 2;
        const diffA = Math.abs(a.finalPrice - midPrice);
        const diffB = Math.abs(b.finalPrice - midPrice);
        return diffA - diffB;
      });

      // Lấy variants trong tolerance
      const nearestVariants = allVariants.filter(v => {
        const diff = Math.abs(v.finalPrice - ((priceRangeMin + priceRangeMax) / 2));
      ...
      if (nearestVariants.length > 0) {
        ...
        nearestVariants.forEach((v: any) => {
          ...
          const finalPrice = v.finalPrice;
          const { hasPromotion } = calculateDiscountPrice(originalPrice, v.product.id, v.product.categoryId, v.product.brandId, activePromotions);
          const variantName = `${v.product.name} - ${storageObj.label || "N/A"} - ${colorObj.label || "Mặc định"}`;
          const originalPriceStr = originalPrice.toLocaleString("vi-VN");
          const finalPriceStr = finalPrice.toLocaleString("vi-VN");
          const stock = v.quantity > 0 ? "Còn hàng" : "Hết hàng";
          const stockColor = v.quantity > 0 ? "#52c41a" : "#ff4d4f";
          ...
          cardsHtml += `
                    <div class="product-card" data-variant-id="${v.id}" data-product-slug="${v.product.slug}" data-variant-name="${variantName}" data-variant-price="${finalPriceStr}đ">
                        <div class="product-image">
                            <img src="https://via.placeholder.com/200" alt="${variantName}" />
                        </div>
                        <div class="product-info">
                            <div class="product-name">${variantName}</div>
                            <div class="product-specs">${storageObj.label || "N/A"}, Màu ${colorObj.label || "Mặc định"}</div>
    ...
    return {
      ...
      html: cardsHtml,
      format: "html_cards",
      message: priceMatchMessage || `Dưới đây là các phiên bản ${products[0]?.name || keyword} hiện có. Bạn muốn chọn phiên bản nào ạ?`,
...
// ============================================================================
const generatePaymentLinkForEmail = async (paymentMethodCode: string, orderId: string, totalAmount: number, orderCode: string): Promise<string | null> => {
  try {
...
const generateOrderCode = (): string => {
  ...
  const randomLetters1 = String.fromCharCode(65 + Math.floor(Math.random() * 26)) + String.fromCharCode(65 + Math.floor(Math.random() * 26));
  ...
  const randomLetters2 = String.fromCharCode(65 + Math.floor(Math.random() * 26)) + String.fromCharCode(65 + Math.floor(Math.random() * 26));
...
const executeCreateOrderRequest = async (args: any) => {
  try {
    ...
    const activePromotions = await getActivePromotionsForChatbot();
    const { finalPrice: finalUnitPrice } = calculateDiscountPrice(Number(variant.price), variant.product.id, variant.product.categoryId, variant.product.brandId, activePromotions);
    ...
    if (isUUID(args.paymentMethodId)) {
      // Nếu là UUID thì tìm theo id
      paymentMethod = await prisma.payment_methods.findUnique({
        where: { id: args.paymentMethodId },
      });
    } else {
      // Nếu không phải UUID → tìm theo code hoặc name
      paymentMethod = await prisma.payment_methods.findFirst({
        where: {
          OR: [
            { code: args.paymentMethodId.toUpperCase() }, // momo → MOMO
            { name: args.paymentMethodId },
          ],
        },
      });
    }
    ...
    try {
      ...
      if (paymentMethod?.code && ["MOMO", "VNPAY", "ZALOPAY"].includes(paymentMethod.code)) {
        paymentLink = await generatePaymentLinkForEmail(paymentMethod.code, order.id, Number(order.totalAmount), order.orderCode);
      }
...
const executeTrackOrder = async (args: any) => {
  try {
    ...
    if (args.orderCode) {
      ...
      where.OR = [{ shippingPhone: args.phone }, { user: { phone: args.phone } }];
    } else {
    ...
    return {
      ...
      message: orders.length === 1 ? `✅ Tôi tìm thấy 1 đơn hàng của bạn. Dưới đây là thông tin chi tiết:` : `✅ Tôi tìm thấy ${orders.length} đơn hàng của bạn. Dưới đây là những đơn gần nhất:`,
    };
  } catch (error: any) {
    ...
    if (msg.role === "user") {
      ...
      if (text.length > 5 && !isConfirmationMessage(text) && !text.includes("chọn phương thức")) {
        ...
        if (parts.length >= 3) {
          ...
          if (text.toLowerCase().includes("phường") || text.toLowerCase().includes("thành phố") || text.toLowerCase().includes("quận") || text.toLowerCase().includes("đường")) {
            result.address = text;
          }
        }
      }
...
Dưới đây là các phương thức thanh toán có sẵn, bạn chọn cách nào ạ:`,
    ...
    if (toolCalls && toolCalls.length > 0) {
      console.log(
        "🚀 LỆNH TỪ AI - ĐANG GỌI TOOL:",
        JSON.stringify(
          toolCalls.map((c) => ({ name: (c as any).function?.name, args: (c as any).function?.arguments })),
          null,
          2,
        ),
      );
