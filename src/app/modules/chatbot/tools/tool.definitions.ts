...
export const CHATBOT_TOOLS = [
  {
    ...
    description: `Tìm kiếm sản phẩm. 
      ...
      - CẤM GỌI TOOL KHI CHƯA CÓ ĐIỀU KIỆN: Nếu khách chỉ hỏi chung chung (VD: "bán điện thoại", "cho xem laptop") mà không nói rõ mức giá, hãng hay nhu cầu -> TUYỆT ĐỐI KHÔNG GỌI TOOL NÀY, BẮT BUỘC hỏi lại khách ngân sách khoảng bao nhiêu hoặc thích hãng nào.
...
[PHỤ KIỆN KHÁC]: Sạc/Pin dự phòng(sac-du-phong), Củ/Cáp sạc(sac-cap), Ốp lưng/bao da(bao-da-op-lung), Dán màn hình(mieng-dan-man-hinh), Bút cảm ứng(but-cam-ung), Webcam(webcam), Hub/USB hub(hub-chuyen-doi), Giá đỡ laptop(gia-do), Balo/túi xách(balo-tui-xach), Miếng lót chuột(mieng-lot-chuot).`,
        ...
        minPrice: { type: "integer", description: "Số nguyên, tuyệt đối không dùng string có ngoặc kép." },
        maxPrice: { type: "integer", description: "Số nguyên, tuyệt đối không dùng string có ngoặc kép." },
        sortBy: {
          type: "string",
...
(QUAN TRỌNG: TRẢ VỀ DƯỚI DẠNG CHUỖI JSON, ví dụ: "{\\"RAM\\":\\"8 GB\\"}")`,
        ...
        limit: { type: "integer", description: "Số lượng kết quả, mặc định 5, tối đa 10." },
      },
  ...
  },
  {
    ...
    description: "Lấy thông số kỹ thuật chi tiết của NHIỀU sản phẩm cùng lúc để so sánh.",
    parameters: {
      type: "object",
      properties: {
        productNames: {
          type: "array",
          items: { type: "string" },
          description: "Danh sách đích danh tên các sản phẩm cần so sánh (VD: ['Galaxy A17 5G', 'Galaxy A26 5G', 'iPhone 15'])"
        }
      },
